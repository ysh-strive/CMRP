...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: Tuple[int, int], patch_size: int, stride_size: int, width: int, layers: int,
        ...
        self.num_x = (input_resolution[1] - patch_size) // stride_size + 1 # 8
        self.num_y = (input_resolution[0] - patch_size) // stride_size + 1 # 24
...
class CLIP(nn.Module):
    ...
    def encode_text_xClip(self, text, return_hidden=True):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]

        pos_emd = self.positional_embedding[:x.size(1), :].type(self.dtype)  # [n_ctx, d_model]
        x = x + pos_emd
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
    ...
    def encode_text_Rap(self, token_type, text, return_hidden=True):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        reg = x.clone()
        pos_emd = self.positional_embedding[:x.size(1), :].type(self.dtype)  # [n_ctx, d_model]
        x = x + pos_emd
        ...
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        t_features_all = self.ln_final(x).type(self.dtype) @ self.text_projection

        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        t_features_cls = t_features_all[torch.arange(t_features_all.shape[0]), text.argmax(dim=-1)]
        t_features_token = t_features_all


        if 'tt' in token_type:
            tt = text.clone()
            tt[:, 0] = 0
            tt[:, text.argmax(dim=-1)] = 0
            y = self.token_embedding(tt).type(self.dtype)
            y = y + pos_emd
            y = y.permute(1, 0, 2)  # NLD -> LND
            y = self.transformer(y)
            y = y.permute(1, 0, 2)  # LND -> NLD
            y_token = self.ln_final(y).type(self.dtype) @ self.text_projection
            # t_features_token = y_token.clone()[:, 1:t_features_all.shape[1]-2, :]
            t_features_token = y_token[:, 1:t_features_all.shape[1] - 1, :]


        else:
            reg[torch.arange(t_features_all.shape[0]), text.argmax(dim=-1)] = 0
            reg[:, 0, :] = 0
            reg = reg + pos_emd
            reg = reg.permute(1, 0, 2)  # NLD -> LND
            reg = self.transformer(reg)
            reg = reg.permute(1, 0, 2)  # LND -> NLD
            reg_token = self.ln_final(reg).type(self.dtype) @ self.text_projection
            t_features_token = reg_token.clone()[:, 1:t_features_all.shape[1] - 1, :]

        i = 0
        text_maks = torch.zeros(text.shape[0], 77)
        for pos in (torch.arange(t_features_all.shape[0]), text.argmax(dim=-1))[1].tolist():
            text_maks[i, 1:pos] = text[i, 1:pos]
            text_maks[i, 1:pos] = 1
            i = i + 1
        t_mask = text_maks.clone()[:, 1:76]

        return t_features_all, t_features_cls, t_features_token, t_mask

    def encode_image_Rap(self, image):
        i_features_all = self.visual(image.type(self.dtype))
        i_features_cls = i_features_all.clone()[:, 0, :]

        i_features_patch = i_features_all[:, 1:, :]

        return i_features_all, i_features_cls, i_features_patch
